...
const valid = { name: "Testkunde", phone: "030123456", email: "test@kunde.de", message: "Innenraum reinigen", privacy: "accepted" };

function serverHarness(deliver) {
  let handle;
  let cleanup;
  const logs = [];
  const writes = [];
  const disk = new Proxy(fs, {
    get(target, key) {
      if (/^(write|append|createWrite|mkdir|rename)/.test(String(key))) {
        return () => { writes.push(key); throw new Error("Persistence forbidden"); };
      }
      return target[key];
    }
  });
  const context = vm.createContext({
    __dirname: path.join(root, "backend"), URL,
    process: { env: {} },
    console: { log() {}, error: (...args) => logs.push(args.join(" ")) },
    setInterval: (callback) => { cleanup = callback; return { unref() {} }; },
    require: (name) => {
      if (name === "http") return { createServer: (callback) => { handle = callback; return { listen() {} }; } };
  ...
  return {
    context, logs, writes, cleanup: () => cleanup(),
    async request(payload, raw = false) {
      const req = new EventEmitter();
      Object.assign(req, { method: "POST", url: "/api/inquiry", headers: { host: "localhost" }, socket: { remoteAddress: "192.0.2.10" }, destroy() {} });
      const headers = {};
      let status;
      let body;
      const res = {
      ...
      req.emit("data", raw ? payload : JSON.stringify(payload));
      req.emit("end");
      await pending;
...
test("inquiries are mailed without any filesystem writes", async () => {
  let received;
  const harness = serverHarness(async (inquiry) => { received = inquiry; return { sent: true }; });
...
test("unconfigured, rejected and failed mail never produce success", async () => {
  for (const deliver of [
    ...
    async () => { throw new Error(`SMTP rejected ${valid.email}: ${valid.message}`); }
  ]) {
...
test("invalid data and honeypot never trigger mail", async () => {
  for (const payload of [null, [], { ...valid, privacy: "" }, { ...valid, email: "a@b.de,c@d.de" }, { ...valid, email: "a@b.de\r\nBcc: other@d.de" }]) {
    const harness = serverHarness(async () => { assert.fail("Unexpected delivery"); });
...
test("rate limiting keeps only expiring pseudonymous counters", async () => {
  ...
  vm.runInContext("for (const [key] of inquiryAttempts) inquiryAttempts.set(key, [Date.now() - RATE_LIMIT_WINDOW - 1])", harness.context);
  harness.cleanup();
...
function mailHarness(env, result = { accepted: ["inhaber@betrieb.de"] }) {
  ...
  const context = vm.createContext({
    ...
    require: () => ({ createTransport: (options) => ({ sendMail: async (message) => { calls.push({ options, message }); return result; } }) }),
    fetch: async (url, options) => { calls.push({ url, options }); return result; }
  });
...
const mailEnv = { SMTP_HOST: "smtp.betrieb.de", SMTP_USER: "user", SMTP_PASS: "secret", MAIL_FROM_EMAIL: "website@betrieb.de", INQUIRY_RECIPIENT: "inhaber@betrieb.de" };
const mailData = { ...valid, id: "test-inquiry-id", createdAt: "2026-08-31T09:00:00Z", service: "Innenraum", message: "<script>alert(1)</script>" };
...
test("missing configuration and placeholder recipients cannot send", async () => {
  for (const env of [{}, { ...mailEnv, INQUIRY_RECIPIENT: "kontakt@kundenname.example" }, { ...mailEnv, MAIL_FROM_EMAIL: "website@example.de" }, { ...mailEnv, MAIL_TRANSPORT: "unknown" }]) {
...
test("form resets only after confirmed mail acceptance; failures preserve input", async () => {
  for (const response of [
    ...
    const context = vm.createContext({
      ...
      document: { addEventListener() {}, querySelector: (selector) => selector === "#inquiry-form" ? form : status },
      window: { location: { search: "" } },
      FormData: class { entries() { return Object.entries(valid); } },
      fetch: async () => {
