...
const forbiddenPublicCopy = [
  {
    pattern: /neu\s+(?:eröffnet|eroeffnet)|neu eröffneter|neu eroeffneter/i,
    label: "unbestätigte Neueröffnungs-Aussage",
  },
  {
    pattern:
      /Go-Live|Prüffassung|Prueffassung|Kundenfreigabe|Kundendaten|Paketstruktur|Leistungsstruktur|vorbereiteter Leistungsbereich|wird noch eingerichtet|wird aktuell .{0,50}(?:abgestimmt|bestätigt)|finaler? Betriebsablauf/i,
    label: "interner Entwicklungsbegriff",
  },
  {
    pattern:
      /\b(?:fuer|spaeter|eroeffnet|Qualitaet|Flaeche|koennen|persoenlich|Rueckmeldung|Einschaetzung|Pruefung|pruefen|Uebergabe|Oeffnungszeiten|gewuenscht\w*|Moeglich\w*|Aussen\w*|Strasse)\b/,
    label: "ASCII-Ersatzschreibweise",
  },
];
...
for (const file of htmlFiles) {
  ...
  if (!/<meta\s+name="description"\s+content="[^"]+"/i.test(html))
    errors.push(`${file}: Meta Description fehlt`);
  ...
  for (const font of ["inter-latin.woff2", "space-grotesk-latin.woff2"]) {
    if (!html.includes(`href="/fonts/${font}" as="font" type="font/woff2" crossorigin`))
      errors.push(`${file}: lokaler Font-Preload fehlt: ${font}`);
  }
  ...
  for (const match of html.matchAll(/data-service-icon="([a-z0-9-]+)"/gi)) {
    if (!fs.existsSync(path.join(frontend, "icons", `${match[1]}.svg`)))
      errors.push(`${file}: Icon fehlt ${match[1]}.svg`);
  }
...
if ((contactHtml.match(/class="contact-channel(?:\s[^"]*)?"/g) || []).length !== 3)
...
if (
  ...
  errors.push("kontakt.html: Standortüberschrift muss bei den Öffnungszeiten neben der Karte stehen");
if (contactHtml.includes("contact-request-notes") || contactHtml.includes("contact-final"))
  errors.push("kontakt.html: doppelte Customer Journey gefunden");
if (contactHtml.includes('class="contact-request-layout"'))
  errors.push("kontakt.html: schmale Formular-Seitenspalte gefunden");
if ((contactHtml.match(/<fieldset class="contact-form__group">/g) || []).length !== 2)
  errors.push("kontakt.html: zwei semantische Formulargruppen erforderlich");
if (
  !contactHtml.includes('aria-describedby="request-required"') ||
  !contactHtml.includes('type="tel" name="phone"')
)
  errors.push("kontakt.html: Formular-Beschreibung oder Telefon-Eingabetyp fehlt");
if (
  fs.existsSync(path.join(frontend, "galerie.html")) ||
  fs.existsSync(path.join(frontend, "js/site-data.js"))
)
  errors.push("Öffentliche Galerie muss entfernt sein");
...
for (const font of ["inter-latin", "inter-latin-ext", "space-grotesk-latin", "space-grotesk-latin-ext"]) {
  ...
  if (!fs.existsSync(file) || fs.readFileSync(file).subarray(0, 4).toString() !== "wOF2")
    errors.push(`WOFF2-Schrift fehlt oder ungültig: ${font}`);
}
...
if (!config.logo || !existsForUrl(config.logo) || !config.logoAlt)
  errors.push("Zentrales Logo oder Alternativtext fehlt");
for (const icon of [
  "circle-check-big",
  "phone",
  "message-circle",
  ...
  if (!fs.existsSync(path.join(frontend, "icons", `${icon}.svg`)))
    errors.push(`Kontakt-/Checklisten-Icon fehlt: ${icon}`);
}
const slugs = new Set();
for (const service of config.services) {
  for (const key of [
    ...
    if (!service[key] || !service[key].length)
      errors.push(`Service ${service.slug || "ohne Slug"}: ${key} fehlt`);
  }
  if (slugs.has(service.slug)) errors.push(`Service-Slug doppelt: ${service.slug}`);
  slugs.add(service.slug);
  if (!fs.existsSync(path.join(frontend, `${service.slug}.html`)))
    errors.push(`Leistungsseite fehlt: ${service.slug}.html`);
  if (service.image && !existsForUrl(service.image)) errors.push(`Service-Bild fehlt: ${service.image}`);
  if (typeof service.active !== "boolean")
    errors.push(`Service ${service.slug}: active muss true oder false sein`);
  if (!/^#[0-9a-f]{6}$/i.test(service.accent || ""))
    errors.push(`Service ${service.slug}: ungültige Akzentfarbe`);
  if (!["lime", "blue", "orange", "violet", "red", "teal", "yellow"].includes(service.theme))
    errors.push(`Service ${service.slug}: ungültiges Farbthema`);
  if (!Array.isArray(service.cardSteps) || service.cardSteps.length < 3 || service.cardSteps.length > 5)
    errors.push(`Service ${service.slug}: drei bis fünf Kartenschritte erforderlich`);
  for (const step of service.cardSteps || []) {
    if (!step.label || !step.icon) errors.push(`Service ${service.slug}: Kartenschritt unvollständig`);
    if (step.icon && !fs.existsSync(path.join(frontend, "icons", `${step.icon}.svg`)))
      errors.push(`Service-Icon fehlt: ${step.icon}.svg`);
  }
}

if (typeof config.indexingEnabled !== "boolean")
  errors.push("indexingEnabled muss ein boolescher Freigabewert sein");
try {
  const url = new URL(config.publicUrl);
  if (url.protocol !== "https:" || url.username || url.password)
    errors.push("publicUrl muss eine sichere öffentliche HTTPS-Adresse sein");
} catch {
  errors.push("publicUrl ist ungültig");
}
for (const file of ["index.html", "kontakt.html"]) {
  const html = fs.readFileSync(path.join(frontend, file), "utf8");
  if (!/<form[^>]+method="post"[^>]+action="\/api\/inquiry"/.test(html))
    errors.push(`${file}: Formular darf keine Kundendaten in die URL schreiben`);
  for (const [name, max] of [
    ...
    const field = html.match(new RegExp(`<(?:input|textarea)[^>]*name="${name}"[^>]*>`))?.[0];
    if (!field?.includes(`maxlength="${max}"`)) errors.push(`${file}: Längenbegrenzung für ${name} fehlt`);
  }
}
